    ...
    <div class="top">
      ...
      <div class="left">
        ...
        <div class="left-bottom">
          ...
          <div class="map" id="professionMap" style="width:4.14rem;height:3rem;"></div>
            ...
            <div class="table-box" id="staff">
              <table id="staff1">
                <tr v-for="(item,index) in teamCount.list" :key="index">
                  <td>{{item.title}}</td>
                  <td v-if="item.createTime=='in'">进</td>
                  <td v-if="item.createTime=='out'">出</td>
                  <td>{{item.inOut_id}}</td>
                </tr>
                <tr></tr>
              </table>
            ...
            <ul id="rightBottom1">
              <li v-for="(item,index) in buildCompany" :key="index">
                ...
                <div class="reality" id="reality">
                  <span>现场实时人数</span>
                  <span class="progress" :style="`width:${(item.kq/item.zc)*1.4}rem`"></span>
                  <span>{{item.kq || 0}}</span>
                </div>
              </li>
            </ul>
      ...
      <!-- <div class="procedure">
        ...
        <div class="float-left">
          <ul style="margin-top:-.1rem">
            <li>
              ...
              <span v-if="contract.workConfirm">{{contract.workConfirm.total}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">共签订:</span>
              <span v-if="contract.workConfirm">{{contract.workConfirm.ht}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">未签订:</span>
              <span v-if="contract.workConfirm">{{contract.workConfirm.wq}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">是否合格:</span>
              <span v-if="contract.workConfirm" :class="contract.workConfirm.hg=='合格'?'font-green':'font-red'">{{contract.workConfirm.hg}}</span>
            </li>
          </ul>
        </div>
        <div
        ...
        >
          ...
          <div class="border" :class="contract.workConfirm.hg=='合格'?'border-green':'border-red'" v-if="contract.workConfirm">
            ...
            <span v-if="contract.workConfirm.bfb.length>0" id="roateBfb1">{{contract.workConfirm.bfb}}</span>
            <span v-else>0</span>
          </div>
        </div>
      </div> -->
      ...
      <!-- <div class="contract">
        ...
        <div class="float-left">
          <ul style="margin-top:-.1rem">
            <li>
              <span class="span-margin">共录入:</span>
              <span v-if="contract.entrance">{{contract.entrance.total}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">共签订:</span>
              <span v-if="contract.entrance">{{contract.entrance.ht}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">未签订:</span>
              <span v-if="contract.entrance">{{contract.entrance.wq}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">是否合格:</span>
              <span v-if="contract.entrance" :class="contract.entrance.hg=='合格'?'font-green':'font-red'">{{contract.entrance.hg}}</span>
            </li>
          </ul>
        </div>
        <div
          class="map float-right" v-if="contract.entrance"
          :class="contract.entrance.hg=='合格'?'qualified':'disqualification'"
        >
          <div class="border" :class="contract.entrance.hg=='合格'?'border-green':'border-red'" v-if="contract.entrance">
            这里要注释的
            ...
            <span v-if="contract.entrance.bfb.length>0" id="roateBfb2">{{contract.entrance.bfb}}</span>
            <span v-else>0</span>
          </div>
        </div>
      </div> -->
      ...
      <!-- <div class="safety">
        ...
        <div class="float-left">
          <ul style="margin-top:-.1rem">
            <li>
              <span class="span-margin">共录入:</span>
              <span v-if="contract.contract">{{contract.contract.total}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">共签订:</span>
              <span v-if="contract.contract">{{contract.contract.ht}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">未签订:</span>
              <span v-if="contract.contract">{{contract.contract.wq}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">是否合格:</span>
              <span v-if="contract.contract" :class="contract.contract.hg=='合格'?'font-green':'font-red'">{{contract.contract.hg}}</span>
            </li>
          </ul>
        </div>
        <div
          class="map float-right" v-if="contract.contract"
          :class="contract.contract.hg=='合格'?'qualified':'disqualification'"
        >
          <div class="border" :class="contract.contract.hg=='合格'?'border-green':'border-red'" v-if="contract.contract">
            这里要注释的
            ...
            <span v-if="contract.contract.bfb.length>0" id="roateBfb3">{{contract.contract.bfb}}</span>
            <span v-else>0</span>
          </div>
        </div>
      </div> -->
      ...
      <!-- <div class="information">
        ...
        <div class="float-left">
          <ul style="margin-top:-.1rem">
            <li>
              <span class="span-margin">共录入:</span>
              <span v-if="contract.exit_pdf">{{contract.exit_pdf.total}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">共签订:</span>
              <span v-if="contract.exit_pdf">{{contract.exit_pdf.ht}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">未签订:</span>
              <span v-if="contract.exit_pdf">{{contract.exit_pdf.wq}}</span>
              <span>人</span>
            </li>
            <li>
              <span class="span-margin">是否合格:</span>
              <span v-if="contract.exit_pdf" :class="contract.exit_pdf.hg=='合格'?'font-green':'font-red'">{{contract.exit_pdf.hg}}</span>
            </li>
          </ul>
        </div>
        <div
          class="map float-right" v-if="contract.exit_pdf"
          :class="contract.exit_pdf.hg=='合格'?'qualified':'disqualification'"
          >
          <div class="border" :class="contract.exit_pdf.hg=='合格'?'border-green':'border-red'" v-if="contract.exit_pdf">
            这里要注释掉的
            ...
            <span v-if="contract.exit_pdf.bfb.length>0" id="roateBfb4">{{contract.exit_pdf.bfb}}</span>
            <span v-else>0</span>
          </div>
        </div>
      </div> -->
    ...
    getWorkType() {
      this.$axios.post(`/api/attendanceRecordApi/getWorkType?projectId=${this.projectId}`).then(
        res => {
          ...
          // debugger
          let temp = [{'value':0,'name':'无数据'}]
          if (res.data.xcgz.length) {
            temp = []
            for (let i = 0; i < res.data.xcgz.length; i++) {
              temp.push(
                {
    ...
    getNearlyEightDays() {
      this.$axios.post(`/api/attendanceRecordApi/getNearlyEightDays?projectId=${this.projectId}`).then(
        res => {
          ...
          let mc = []
          for (let i = 0; i < res.data.list.length; i++) {
            date.unshift(res.data.list[i].date)
            count.unshift(res.data.list[i].count)
            wc.unshift(res.data.list[i].workerCheck)
            mc.unshift(res.data.list[i].managerCheck)
          }
          setTimeout(()=>{
...
#labour .top .left .left-top .la-subjindu {
      ...
      height: 0.24rem;
      width: 0%;
      background: linear-gradient(to right, #009b41, #00fe15);
